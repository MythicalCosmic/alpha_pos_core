...
def test_local_global_identity_resolves_to_bound_node_branch(identity_branch):
...
def test_local_concrete_foreign_identity_does_not_resolve_to_node_branch():
    ...
    actor = SimpleNamespace(branch_id='branch-b')

    assert resolve_actor_branch(actor) == 'branch-b'


@override_settings(DEPLOYMENT_MODE='local', BRANCH_ID='branch-a')
def test_local_cloud_cashier_can_read_and_end_own_operational_shift():
    from core.shifts.service import ShiftService

    cashier = _staff(branch='cloud', role='CASHIER')
    shift = _shift(cashier, branch='branch-a')

    result, status = ShiftService.get(shift.id, actor=cashier)
    assert status == 200, result

    result, status = ShiftService.end_shift(
        shift.id, cashier.id, 'local close', actor=cashier,
    )
    assert status == 200, result
...
@override_settings(DEPLOYMENT_MODE='local', BRANCH_ID='branch-a')
def test_local_concrete_foreign_cashier_cannot_access_operational_shift():
    from core.shifts.service import ShiftService

    cashier = _staff(branch='branch-b', role='CASHIER')
    shift = _shift(cashier, branch='branch-a')

    _result, status = ShiftService.get(shift.id, actor=cashier)
